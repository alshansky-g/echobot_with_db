...
async def change_user_alive_status(
    ...
    logger.info("Updated `is_alive` status to `%s` for user %d", is_alive,
                user_id)
...
async def change_user_banned_status_by_username(
    ...
    logger.info("Updated `banned` status to `%s` for username %s", banned,
                username)
...
async def get_user_lang(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with `user_id`=%s has the language %s",
                    user_id, row[0])
    else:
        logger.warning("No user with `user_id`=%s found in the database",
                       user_id)
...
async def get_user_alive_status(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with `user_id`=%s has the is_alive status is %s",
                    user_id, row[0])
    else:
        logger.warning("No user with `user_id`=%s found in the databse",
                       user_id)
    return row[0] if row else None
...
async def get_user_banned_status_by_id(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with `user_id`=%s has the banned status is %s",
                    user_id, row[0])
    else:
        logger.warning("No user with `user_id`=%s found in the database",
                       user_id)
...
async def get_user_banned_status_by_username(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with `username`=%s has the banned status is %s",
                    username, row[0])
    else:
        logger.warning("No user with `username`=%s found in the database",
                       username)
    return row[0] if row else None
...
async def get_user_role(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with `user_id`=%s has the role is %s",
                    user_id, row[0])
    else:
        logger.warning("No user with `user_id`=%s found in the database")
